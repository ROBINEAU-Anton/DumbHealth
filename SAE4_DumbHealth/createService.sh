...
read -p "Is your service using Axios ? [y/N]" ax
if [ $ax = "y" | $ax = "Y" ];
then
yarn add -D axios
fi

read -p "Is your service using Mongoose ? [y/N]" mdb
if [ $mdb = "y" | $mdb = "Y" ];
then
yarn add -D mongoose
fi
...
# TODO: Install tests dependancies
...
# Create the .env and .env.template files
echo "APP_PORT=$appPort" > .env
cp .env .env.template

# Create the server.js tempalte file
echo "\"use strict\"
